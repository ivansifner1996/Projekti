...
def listaj():
	...
	if request.method == 'POST':
		...
		if button is not None:
			print("Id buttona je", button)
			#db.execute("DELETE FROM Zapisnik") za brisanje svih
			db.execute("DELETE FROM Zapisnik WHERE id == "+button+"")
			#db.execute("DELETE FROM Zapisnik WHERE id == "+button+"") za sve osim tog filma
			db.commit()
			podaciz = db.execute("SELECT naziv_filma, godina, datum_pregleda, id FROM Zapisnik Where korisnik_mail=:korisnik_mail", {"korisnik_mail":korisnik_mail}).fetchall()
			return render_template('lista_filmova.html', value = podaciz)
		elif godina != "":
			godina = int(godina)
